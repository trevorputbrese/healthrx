package com.healthrx.agent.financial.messaging;
...
/**
 * The agent's own durable queue with a single narrow binding — it senses only
 * PriorAuthorizationApproved, the moment a case might need financial assistance.
 */
@Configuration
public class RabbitConfig {
    ...
    Binding agentBinding(@Value("${healthrx.events.queue}") String queue,
        ...
        return BindingBuilder.bind(new Queue(queue)).to(new TopicExchange(exchange))
                .with(prefix + "PriorAuthorizationApproved");
    }
}
