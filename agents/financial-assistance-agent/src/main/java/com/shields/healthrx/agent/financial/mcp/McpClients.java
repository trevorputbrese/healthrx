package com.shields.healthrx.agent.financial.mcp;
...
/**
 * MCP clients to the gateway: every read (Postgres MCP server) and every write (HealthRx-embedded
 * action tools) is an audited tool call through healthrx-mcp-gw. Both clients carry the agent's
 * interim identity headers (X-Agent-Id/X-Agent-Key, forwarded upstream by the gateway) and a
 * distinctive User-Agent for the mcp.audit logs.
 */
@Configuration
public class McpClients {

    public static final String USER_AGENT = "healthrx-financial-assistance-agent";
