 ...
 * triage, phase-3-design.md §6), BenefitsInvestigationStarted (the benefits beat that submits
 * the PA), and PriorAuthorizationSubmitted (the payer follow-up beat); stuck referrals are
 * found by the scan, not by events.
 */
@Configuration
public class RabbitConfig {
    ...
    @Bean
    Binding benefitsStartedBinding(@Value("${healthrx.events.queue}") String queue,
            @Value("${healthrx.events.exchange}") String exchange,
            @Value("${healthrx.events.routing-prefix}") String prefix) {
        return BindingBuilder.bind(new Queue(queue)).to(new TopicExchange(exchange))
                .with(prefix + "BenefitsInvestigationStarted");
    }
}
