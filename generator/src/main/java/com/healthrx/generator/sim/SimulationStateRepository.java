...
public class SimulationStateRepository {
    ...
    /**
     * Advances the clock only while the simulation is still enabled — a tick that read the
     * pre-pause state must not write its advance after a concurrent pause/reset landed (a demo
     * reset racing an in-flight tick could otherwise get its freshly-anchored clock clobbered
     * by one last stale advance). Returns whether the advance applied.
     */
    public boolean advance(Instant newInstant, Instant updatedAt) {
        return jdbc.update(
                "update simulation_state set current_instant = ?, updated_at = ? where id = 1 and enabled",
                OffsetDateTime.ofInstant(newInstant, ZoneOffset.UTC),
                OffsetDateTime.ofInstant(updatedAt, ZoneOffset.UTC)) > 0;
    }
